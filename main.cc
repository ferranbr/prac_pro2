...
int main () {
    ...
    while (command != "fin") {
        ...
        if (command == "alta_bici" or command == "ab") {
            ...
            else estaciones[id_est].sacar_bicis();
        }
            ...
            if (ests.existe_bicicleta(id_bici)) cout << "error: la bici ya existe" << endl;
            else if(!ests.hay_hueco()) cout << "error: la bici no cabe" << endl;
            else {
                string id_est_correcto;
                int plazas,recs;
                ests.meter_bici(id_bici,id_est_correcto,plazas,recs,ids_estaciones);
            }
        }
